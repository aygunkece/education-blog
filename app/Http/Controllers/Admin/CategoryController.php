...
class CategoryController extends Controller
{
    ...
    public function changeFeatureStatus(Request $request)
    {
        ...
        $oldFeatureStatus = $category->feature_status;

        $category->feature_status = !$category->feature_status;
        $category->save();

        $featureStatusText = ($oldFeatureStatus == 1 ? "Aktif" : "Pasif") . "'ten " . ($category->feature_status == 1 ? "Aktif" : "Pasif");

        alert()->success('Başarılı', $category->name . " feature status değeri " . $featureStatusText . " olarak güncellendi")->showConfirmButton('Tamam', '#3085d6')->autoClose(5000);
